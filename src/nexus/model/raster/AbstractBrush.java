...
public abstract class AbstractBrush implements Brush
{
	...
	public void fill(Chunk chunk, Vector3i from, Vector3i to, String type)
	{
		...
		for (int z = rc.y; z < rc.y + rc.height; z++) {
			for (int x = rc.x; x < rc.x + rc.width; x++) {
				for (int y = y1; y < y2; y++) {
					setBlock(chunk, x - wx, y, z - wz, type);
				}
			}
		}
	}
	...
	public void fillAirBelow(Chunk chunk, Rectangle fullRc, int y1, String type)
	{
		...
		Rectangle rc = getIntersectionArea(chunk, x1, z1, x2, z2);
        int wx = chunk.x * chunk.getChunkSizeX();
        int wz = chunk.z * chunk.getChunkSizeZ();
	...
	@Override
	public void clearAbove(Chunk chunk, Rectangle rc, int y1)
	{
		int x1 = rc.x;
